 ...
 * three linked lists. Threads in the first and second list are scheduled using 
...
public class MultilevelQueueScheduler extends Scheduler {
    ...
    public boolean timerInterrupt() {
        ...
        if(current_queue == 0){
            ...
            if(interrupt_interval_counter >= 16){
                current_queue = 1;
                interrupt_interval_counter = 0;
            }
        }
       ...
       /* Implement quanta within each queue */
        if(queue_num == 0){
            System.out.println("\nCURRENT QUEUE: " + queue_num);
            System.out.println("\nYIELDING AT: " + Machine.timer().getTime());
            // Yield every 500 ticks for first queue. (i.e. every time timerInterrupt() is invoked)
            return true;
        }
        else if(queue_num == 1){
            if(interrupt_interval_counter % 2 == 0){
                System.out.println("\nCURRENT QUEUE: " + queue_num);
                System.out.println("\nYIELDING AT: " + Machine.timer().getTime());
                // Yield every 1000 ticks for second queue. (i.e. every two invocations of timerInterrupt())
                return true;
            }
            else {
                return false;
        ...
        public void print() {
            ...
            for(int i = 0; i < wait_queue.size(); ++i){
                for(int j = 0; j < wait_queue.get(i).size(); ++j){
                    System.out.println(wait_queue.get(i).get(j));
                }
            }
        }
