 ...
 * allows multiple threads to run concurrently. To create a new thread of
 * execution, first declare a class that implements the <tt>Runnable</tt>
 * interface. That class then implements the <tt>run</tt> method. An instance of
 * the class can then be allocated, passed as an argument when creating
 * <tt>KThread</tt>, and forked. For example, a thread that computes pi could be
 * written as follows:
 * <p>
 * <blockquote>
 * 
...
public class KThread {
  ...
  public KThread() {
    ...
    if (currentThread != null) {
      tcb = new TCB(); // create new thread control block for this thread
    } else {
   ...
   * method returns, but it may also be called directly. The current thread
   * cannot be immediately destroyed because its stack and other execution state
   * are still in use. Instead, this thread will be destroyed automatically by
   * the next thread to run, when it is safe to delete this thread.
   */
  public static void finish() {
    ...
    while (!currentThread.joinQueue.isEmpty()) {
      KThread parentThread = currentThread.joinQueue.removeFirst();
      if (parentThread.status != statusReady) {
        parentThread.ready();
      }
    }
  ...
  public void join() {
    ...
    boolean intStatus = Machine.interrupt().disable();
    joinQueue.add(KThread.currentThread());
    KThread.sleep();
    Machine.interrupt().restore(intStatus);
  }
...
*/
  ...
  /**
   * Gets whether the thread is currently blocked.
   */
